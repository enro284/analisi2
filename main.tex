...
\begin{center}    
\begin{tikzpicture}[node distance = \linewidth/4, centered]
    
    \node (esatto){\textbf{esatto}};
    \node (chiuso)[right = of esatto]{\textbf{chiuso}};
    \node (conservativo)[right = of chiuso]{\textbf{conservativo}};
    
    \draw[double, <->] (conservativo) to node[midway, above]{sempre} (chiuso);
    \draw[double, ->] (esatto) to node[midway, above]{sempre} (chiuso);
    \draw[double, ->] (chiuso) to[bend left=20] node[midway, below, text width=5cm, align=center]{$A$ semplicemente connesso (in particolare se $A$ è stellato rispetto a qulche suo punto)} (esatto);
\end{tikzpicture}
\end{center}
